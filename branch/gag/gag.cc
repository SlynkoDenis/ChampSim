...
std::map<O3_CPU*, std::array<champsim::msl::fwcounter<COUNTER_BITS>, GLOBAL_PHT_SIZE>> global_pht;
} // namespace
...
uint8_t O3_CPU::predict_branch(uint64_t ip) {
    auto value = ::global_pht[this][::global_bhr.to_ulong()];
    return value.value() >= (value.maximum / 2);
}

void O3_CPU::last_branch_result(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type) {
    // update PHT
    ::global_pht[this][::global_bhr.to_ulong()] += taken ? 1 : -1;
    // update BHR
    ::global_bhr <<= 1;
    ::global_bhr[0] = taken;
}
